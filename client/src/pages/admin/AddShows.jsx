  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <div className='overflow-x-auto pb-4'>
        <div className='group flex felx-wrap gap-4 mt-4 w-max'>
      ...
      {/* Show Price Input */}
      <div className='nt-8 max-w-xs'>
        <label className='blck text-sm font-mediium mb-2'> Show Price </label>
            ...
            {Object.entries(dateTimeSelection).map(([date, times])=>(
              <li key={date}>
                ...
                <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                  {times.map((time)=>(
                    <div key={time} className='border border-gray-600 px-2 py-1 flex items-center rounded'>
                      <span>{time}</span>
                      <DeleteIcon onClick={()=> handleRemoveTime(date,time)} width={25} className='ml-2 text-red-500 hover:text-red-700 cursor-pointer' />
                    </div>
                  ))}
                </div>
              </li>
